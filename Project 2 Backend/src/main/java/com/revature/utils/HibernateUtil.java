...
public class HibernateUtil {
	...
	// We use the SessionFactory interface in order to get a Session.
		...
		// credentials etc. are stored.
		private static SessionFactory sf = new Configuration().configure("hibernate.cfg.xml").buildSessionFactory();

		// get Session objects from our SessionFactory
		// We only want one session in this application - hence private static - it's a
		// singleton!
		private static Session ses;

		// This method actually gets us a Session object using the SessionFactory
		// interface.
